...
\begin{equation}
  ...
  \label{eq:base}
\end{equation}
where $\rho$ and $\nu$ are density and mixing coefficient, respectively. $u_i$ and $x_i$ are 3-dimensional velocity and coordinate following Einstein's notation rule.
Imcompressive formulation is adopted, and density is ignored in the horizontal diffusion, so each $F^{u_i, x_j}$ is represented as follows:
\begin{equation*}
...
\vspace{-5pt}
\begin{equation}
  \Delta \left( \sum_{i,j} V_{i+\frac{1}{2},j,k} u_{i+\frac{1}{2},j,k} \right) = \sum_{i,j} V_{i+\frac{1}{2},j,k} \Delta u_{i+\frac{1}{2},j,k} = 0,
\end{equation}
...
\vspace{-5pt}
\begin{equation}
  \Delta \left( \sum_{i,j} V_{i,j+\frac{1}{2},k} v_{i,j+\frac{1}{2},k} \right) = \sum_{i,j} V_{i,j+\frac{1}{2},k} \Delta v_{i,j+\frac{1}{2},k} = 0
\end{equation}
\vspace{-5pt}
\begin{equation}
  \Delta \left( \sum_{i,j} V_{i,j,k+\frac{1}{2}} w_{i,j,k+\frac{1}{2}} \right) = \sum_{i,j} V_{i,j,k+\frac{1}{2}} \Delta w_{i,j,k+\frac{1}{2}} = 0
\end{equation}
...
In the discritization of the third terms in (\ref{eq:tensu}), (\ref{eq:tensv}), we adopt a similar conservative way as follows:
\vspace{-5pt}
\begin{equation}
...
In the LFRic's source code, we use $W3$ function spaces for variables placed on the center of the cell, while we use $W1$ or shifted $W2H$ function space for variables placed on the edges of the cell.
Each degree of freedom (dof) in $W1$ and shifted $W2H$ function space is placed as shown in Figure\ref{fig:dof}.
...
\begin{tabular}{|c|c|c|c|}
 ...
 \hline\hline
 $F^{ux}$ & $(i,j,k)$                         & uflux\_w3                        & $W3$                           \\ \hline
 $F^{uy}$ & $(i+\frac{1}{2},j+\frac{1}{2},k)$ & \multirow{2}{*}{uflux\_w1}       & \multirow{2}{*}{$W1$}          \\ \cline{1-2}
 $F^{uz*}$& $(i+\frac{1}{2},j,k+\frac{1}{2})$ &                                  &                                \\ \hline
 $F^{vy}$ & $(i,j,k)$                         & vflux\_w3                        & $W3$                           \\ \hline
 $F^{vx}$ & $(i+\frac{1}{2},j+\frac{1}{2},k)$ & \multirow{2}{*}{vflux\_w1}       & \multirow{2}{*}{$W1$}          \\ \cline{1-2}
 $F^{vz*}$& $(i,j+\frac{1}{2},k+\frac{1}{2})$ &                                  &                                \\ \hline
 $F^{wx}$ & $(i+\frac{1}{2},j,k+\frac{1}{2})$ & \multirow{2}{*}{wflux\_sh\_w2h}  & \multirow{2}{*}{shifted $W2H$} \\ \cline{1-2}
...
\begin{subfigure}{0.3\linewidth}
  ...
  \begin{tikzpicture}
    ...
    \node at (3.4,5.4) {12};
  \end{tikzpicture}
  \caption{placement of each dof in W1 space}
  \label{fig:dof_w1}
\end{subfigure}
\hspace{15pt}
\begin{subfigure}{0.3\linewidth}
  ...
  \begin{tikzpicture}
    ...
    \node at (3.4,1.4) {4};
  \end{tikzpicture}
  \caption{placement of each dof in shifted W2H space}
